    ...
    public function getJsonData(){
        return response()->json(
            [
                ...
                // 'postList' => post::with('admins')->get(),


                // $posts = post::whereIn('type',['news','popular'])->orderBy('id','DESC')->get();

                'postList' => post::whereIn('type',['news','popular'])->orderBy('id','DESC')->with('admin')->get(),

                // 'postList' => post::with('admin')->get(),


                
                'totalPosts'=> DB::table('posts')->whereNotIn('type',['bulletin'])->orderBy('id','DESC')->get()->count(),
                'bulletinList' => post::where('type','bulletin')->orderBy('id','DESC')->with('admin')->get(),
                
                'pressnoteList' => Pressnote::orderBy('id','DESC')->with('admin')->get(),
    ...
    //    return  $rest = substr($request->body, 0, 1); // returns "de"
       ...
       if($request->mul_images == 'null')
        ...
        if(strpos($mystring, $youtube1) !== false){
            ...
            return response()->json(
                [
                    'message'=>'New Post Bulleting Added...',
                    'status' => 0,
                    
                    'postList' => Post::whereNotIn('type',['bulletin'])->orderBy('id','DESC')->with('admin')->get(),
                    'pressnoteList' => Pressnote::orderBy('id','DESC')->with('admin')->get(),
